...
using namespace std;
/* Topics:
 * 		1) Guaranteeing thread join with RAII for Exception Handling
 */
...
public:
	...
	explicit thread_kerveros(thread& t1) : _t1(std::move(t1)) {cout<<"constucting with lvalue"<<"\n";};
	explicit thread_kerveros(thread&& t1) : _t1(std::move(t1)) { cout<<"Constructing with rvalue"<<"\n";};
...
int main() {
	...
	// -- Issue : Exception thrown before we reach detach or join
